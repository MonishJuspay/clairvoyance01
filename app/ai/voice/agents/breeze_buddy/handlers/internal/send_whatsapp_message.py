...
async def send_whatsapp_message(
    ...
    raw_recipient = str(args.get("recipient_phone_number") or "").strip()
    message_type = str(args.get("message_type") or "").strip().lower()
    ...
    configured_template_name = (
        str(metadata.get("template_name") or "").strip() or None
    )
    ...
    if message_type == _TEMPLATE_MESSAGE_TYPE:
        ...
        if requested_template_name != configured_template_name:
            return {
                "status": "error",
                "whatsapp_sent": False,
                "message": (
        ...
        if (
            "values" not in args
            or values is None
            or not _values_count_is_valid(values, function_config)
        ):
            return {
                "status": "error",
                "whatsapp_sent": False,
